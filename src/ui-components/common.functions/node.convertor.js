import {getStorage, setStorage} from "../storage-options/storage";
...
// Boolean parser, return whatever value wanted for true and false boolean values!
export function booleanParser(value){
...
export function checkboxSelection(value, setState, storageId){
  var selectedCount = getStorage(storageId);
  if(value){
    const updatedCount = Number(selectedCount) + 1
    setStorage(storageId, updatedCount)
